...
#if RELEASE
#if BETA
#define VERSION crypt_str("pummed | ")
#else
#define VERSION crypt_str("pummed | ")
#endif
#else
...
void misc::watermark()
{
	...
	auto net_channel = m_engine()->GetNetChannelInfo(); // Receiving ping may differ from cheat base
	auto delay = (int)(net_channel ? net_channel->GetLatency(FLOW_INCOMING) : 0);

	std::string  infoBuf[255];
	DWORD  bufCharCount = 255;
	IFH(GetUserName)((LPSTR)infoBuf, &bufCharCount);

	std::string username = infoBuf->c_str();

	auto watermark = VERSION + username + crypt_str(" | ") + crypt_str("ping: ") + std::to_string(delay) + crypt_str("ms | ") + g_ctx.globals.time;
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			if (!strcmp(server, crypt_str("loopback")))
				server = crypt_str("local");
			else if (m_gamerules()->m_bIsValveDS())
				server = crypt_str("valve");

			auto tickrate = std::to_string((int)(1.0f / m_globals()->m_intervalpertick));
			watermark = VERSION + username + crypt_str(" | ") + server + crypt_str(" | ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + tickrate + crypt_str(" tick | ") + g_ctx.globals.time;
		}
	}

	auto box_width = render::get().text_width(fonts[NAME], watermark.c_str()) + 10;

	render::get().rect_filled(width - 10 - box_width, 10, box_width, 1, Color(111, 45, 159, 255));
	...
	render::get().rect_filled(width - 10 - box_width, 11, box_width, 18, Color(0, 0, 0, 150));
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("pummed - stay with us or lose the game."),
		crypt_str("Get good. Get pummed."),
		crypt_str("pummed - just the best."),
		crypt_str("Go to another level with pummed."),
	};
...
void misc::anti_defensive()
{
	static auto is_command_set = g_cfg.misc.anti_defensive;
