...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	else
	{
		...
		if (g_cfg.antiaim.type[type].desync)
		{
			...
			if (type == ANTIAIM_RAGE && g_ctx.local()->m_vecVelocity().Length() <= 5.0f && g_cfg.antiaim.lby_type)
				desync_delta *= 2.0f;
			else
			{
				if (!flip)
			...
			if (type == ANTIAIM_RAGE && g_ctx.local()->m_vecVelocity().Length() >= 5.0f && type != ANTIAIM_LEGIT || !g_cfg.antiaim.lby_type)
			{
				if (!flip)
