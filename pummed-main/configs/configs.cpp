...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.antiaim.flicktick, 5, crypt_str("Antiaim.flicktick"));
	setup_item(&g_cfg.antiaim.flick, false, crypt_str("Antiaim.fake_flick"));
	...
	setup_item(&g_cfg.esp.scope_col, false, crypt_str("Esp.scope_col"));
	setup_item(&g_cfg.esp.scope_color, Color(255, 255, 255), crypt_str("Esp.scope_color"));
	...
	setup_item(&g_cfg.misc.anti_defensive, false, crypt_str("Misc.anti_defensive"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\pummed\\Configs\\") + config;
		}
