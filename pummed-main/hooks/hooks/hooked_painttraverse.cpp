...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (reload_fonts())
	{
		...
		fonts[NAME] = create_font(crypt_str("Arial"), 12, 400, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	...
	if (panel_id == panel)
	{
		...
		if (g_ctx.available())
		{
			...
			if (g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_SCOPE] && g_ctx.globals.scoped && weapon->is_sniper())
			{
				...
				auto color = Color(g_cfg.esp.scope_color.r(), g_cfg.esp.scope_color.g(), g_cfg.esp.scope_color.b(), 255);
				auto end_color = Color(g_cfg.esp.scope_color.r(), g_cfg.esp.scope_color.g(), g_cfg.esp.scope_color.b(), 0);
